...
def initialize_model(model_path: str = None, preprocessor_path: str = None):
    ...
    if model_path is None:
        ...
        if model_dir.exists():
            ...
            preprocessors = sorted(model_dir.glob("baseline_preprocessor_*.pkl"), reverse=True)
    ...
    if model_path and preprocessor_path:
        ...
        raise FileNotFoundError("Aucun modèle trouvé. Spécifiez model_path et preprocessor_path.")


def create_plotly_curve(
    dose_response: pd.DataFrame,
    optimal_n: Optional[float] = None,
    title: str = "Courbe de réponse azote-rendement"
) -> go.Figure:
    """
    ...
    if optimal_n is not None:
        optimal_yield = dose_response[dose_response["n_dose"] == optimal_n]["predicted_yield"].values[0]
        fig.add_trace(go.Scatter(
            x=[optimal_n],
            y=[optimal_yield],
            mode='markers',
    ...
    fig.update_layout(
        ...
        yaxis_title="Rendement prédit (t/ha)",
...
def create_input_table_html(features: Dict, nitrogen_range: List[float]) -> str:
    ...
    <div style="margin-top: 20px;">
        <h3 style="color: #2E86AB;">📋 Valeurs d'entrée utilisées</h3>
        <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
            <thead>
                <tr style="background-color: #2E86AB; color: white;">
                    <th style="padding: 10px; text-align: left; border: 1px solid #ddd;">Paramètre</th>
    ...
    html += f"""
                <tr style="border-bottom: 1px solid #ddd; background-color: #f9f9f9;">
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Plage d'azote testée</strong></td>
...
<head>
    ...
    <title>Prédiction Rendement Maïs - {timestamp}</title>
    <style>
        body {{
...
<body>
    <div class="header">
        <h1>🌽 Prédiction de Rendement du Maïs</h1>
        <p>Modèle de réponse à l'azote | Généré le {datetime.now().strftime("%d/%m/%Y à %H:%M:%S")}</p>
    </div>

    <div class="section">
        <h2>📈 Courbe de réponse azote-rendement</h2>
        {plotly_html}
    </div>

    <div class="section">
        <div class="optimal-box">
            <h3>⭐ Dose optimale recommandée</h3>
            <div class="optimal-grid">
                <div class="optimal-item">
                    <div class="label">Dose d'azote optimale</div>
                    <div class="value">{optimal_info['optimal_n_kg_ha']:.0f} kg N/ha</div>
                </div>
                <div class="optimal-item">
                    <div class="label">Rendement prédit</div>
                    <div class="value">{optimal_info['predicted_yield_t_ha']:.2f} t/ha</div>
                </div>
                <div class="optimal-item">
                    <div class="label">Revenu net</div>
                    <div class="value">{optimal_info['net_revenue_per_ha']:.0f} $/ha</div>
                </div>
                <div class="optimal-item">
                    <div class="label">Coût fertilisation</div>
    ...
    <div class="section">
        ...
        <table>
            <thead>
                <tr>
                    <th>Dose N (kg/ha)</th>
                    <th>Rendement prédit (t/ha)</th>
                </tr>
            </thead>
            <tbody>
                {''.join([f'<tr><td>{row["n_dose"]}</td><td>{row["predicted_yield"]:.3f}</td></tr>'
    ...
    <div class="footer">
        <p>Ce rapport a été généré automatiquement par le système de prédiction de rendement du maïs.</p>
        <p>Modèle version 1.0 | Toutes les données sont embarquées dans ce fichier HTML.</p>
    </div>
...
def predict_and_visualize(
    ...
    fig = create_plotly_curve(
        ...
        title="Courbe de réponse azote-rendement"
    )
    ...
    <div style="background: linear-gradient(135deg, #A23B72 0%, #F18F01 100%);
        ...
        <h3 style="margin: 0 0 15px 0;">⭐ Recommandation optimale</h3>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 15px;">
            ...
            <div style="background: rgba(255,255,255,0.2); padding: 15px; border-radius: 5px;">
                <div style="font-size: 12px; opacity: 0.9;">Rendement prédit</div>
...
def create_gradio_interface():
    ...
    if predictor is None:
        raise RuntimeError("Le modèle n'est pas initialisé. Appelez initialize_model() d'abord.")

    # Get feature schema
    schema = predictor.describe_features()

    with gr.Blocks(
        title="Prédiction Rendement Maïs",
        theme=gr.themes.Soft(primary_hue="blue", secondary_hue="pink")
    ) as interface:

        gr.Markdown("""
        # 🌽 Prédiction du Rendement du Maïs
        ### Modèle de réponse à la fertilisation azotée

        Entrez les caractéristiques de votre parcelle pour obtenir une courbe de réponse dose-rendement
        et une recommandation de fertilisation optimale.
        """)

        with gr.Tabs():
            # Tab 1: Static Features
            with gr.Tab("📍 Caractéristiques statiques"):
                with gr.Row():
                    with gr.Column():
                        ...
                        for feature in schema['features']['static']:
                            if feature['type'] == 'numeric':
                                inputs_static[feature['name']] = gr.Slider(
                                    minimum=feature['min'],
                                    maximum=feature['max'],
                                ...
                                choices = [(opt['label'], opt['value']) for opt in feature['options']]
            ...
            with gr.Tab("🌦️ Données météo (optionnel)"):
                gr.Markdown("""
                **Format CSV requis:** Les fichiers CSV doivent contenir les colonnes météo journalières.
                - **Pré-semis:** 30 jours avant le semis
                - **Saison de croissance:** Jusqu'à 200 jours après le semis

                Si aucun fichier n'est fourni, le modèle utilisera des valeurs par défaut.
                """)

                with gr.Row():
                    weather_csv_pre = gr.File(
                        label="📄 CSV météo pré-semis (30 jours)",
                        file_types=[".csv"],
                        type="filepath"
                    )
                    weather_csv_growing = gr.File(
                        label="📄 CSV météo saison de croissance (200 jours)",
                        file_types=[".csv"],
                        type="filepath"
                    )

            # Tab 3: Nitrogen & Economics
            with gr.Tab("💰 Azote et économie"):
                with gr.Row():
                    with gr.Column():
                        gr.Markdown("### Plage de doses d'azote à tester")
                        n_min = gr.Slider(
                            minimum=0,
                            maximum=150,
                            value=0,
                        ...
                        n_step = gr.Slider(
                            ...
                            value=25,
                            step=5,
                            label="Pas d'incrémentation (kg N/ha)"
                        )
                    ...
                    with gr.Column():
                        ...
                        n_price = gr.Number(
                            ...
                            info="Coût de l'engrais azoté par kg de N"
                        )
                        grain_price = gr.Number(
                            value=0.20,
                            label="Prix du grain ($/kg)",
                            info="Prix de vente du maïs grain"
                        )

        # Predict button
        predict_btn = gr.Button("🚀 Calculer la courbe de réponse", variant="primary", size="lg")
        ...
        with gr.Row():
            with gr.Column(scale=2):
                plot_output = gr.Plot(label="Courbe de réponse azote-rendement")

            with gr.Column(scale=1):
                optimal_output = gr.HTML(label="Recommandation optimale")

        with gr.Accordion("📋 Détails des entrées", open=False):
            input_summary_output = gr.HTML()

        # Export button
        gr.Markdown("### 💾 Exporter les résultats")
        export_btn = gr.File(label="Télécharger le rapport HTML complet", type="filepath")
        ...
        all_inputs = [
            ...
            n_min, n_max, n_step,
            n_price, grain_price,
            weather_csv_pre, weather_csv_growing
        ]
        ...
        gr.Markdown("""
        ---
        **Note:** Ce modèle est basé sur des données expérimentales et fournit une estimation.
        Les recommandations doivent être adaptées aux conditions locales spécifiques.
        """)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Application web de prédiction du rendement du maïs")
    parser.add_argument("--model", type=str, help="Chemin vers le fichier .keras du modèle")
    parser.add_argument("--preprocessor", type=str, help="Chemin vers le fichier .pkl du préprocesseur")
    ...
    # Launch
    print(f"🚀 Lancement de l'application sur http://{args.host}:{args.port}")
    interface.launch(
